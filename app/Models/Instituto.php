...
class Instituto extends Model
{
    ...
    public static function getCronogramaInstitutos($region_id = NULL)
    {
        ...
        $data = Instituto::leftjoin('t_regiones', 't_regiones.id', '=', 't_institutos.region_id')
            ...
            ->orderBy('t_institutos.region_id', 'asc')
            ->selectRaw('t_institutos.*, t_regiones.region,t_institutos_fortalecimien.proceso_licenciamiento,t_institutos_fortalecimien.fecha_max_presentacion,cantPrograms.cant_programs, ROW_NUMBER() OVER(ORDER BY t_institutos.updated_at DESC) AS row')
            ->orderBy('row', 'asc')->get();
